...
class ApproveAppealService < BaseService
  ...
  def undo_strike_action!
    ...
    when 'mark_statuses_with_spoiler_text'
      undo_mark_statuses_with_spoiler_text!
    when 'sensitive'
      undo_sensitive!
    when 'silence'
      undo_silence!
  ...
  def undo_mark_statuses_with_spoiler_text!
    representative_account = Account.representative
    @strike.statuses.each do |status|
      UpdateStatusService.new.call(status, representative_account.id, spoiler_text: '', sensitive: false) if status.spoiler_text?
    end
  end
